...
print("=" * 60)
print("GENERADOR DE INSIGHTS TOKEN PERMANENTE - AUTOGOAL (auto-escribe .env)")
print("=" * 60)
...
# Paso 4: escribir en .env (reemplazando solo la linea INSIGHTS_TOKEN)
print("\n[4/4] Escribiendo el token en .env...")
contenido = ARCHIVO_ENV.read_text(encoding="utf-8")
nueva_linea = f"INSIGHTS_TOKEN={page_token}"
if re.search(r"^INSIGHTS_TOKEN=.*$", contenido, flags=re.MULTILINE):
    contenido = re.sub(r"^INSIGHTS_TOKEN=.*$", nueva_linea, contenido, flags=re.MULTILINE)
else:
...
m = re.search(r"^INSIGHTS_TOKEN=(.*)$", releido, flags=re.MULTILINE)
ok_escrito = m and m.group(1).strip() == page_token

print("\n" + "=" * 60)
if ok_escrito:
    print("EXITO: token PERMANENTE (PAGE, expires_at=0) escrito en .env como INSIGHTS_TOKEN.")
    print("PAGE_ACCESS_TOKEN no se ha tocado. Siguiente: actualizar el Secret INSIGHTS_TOKEN en GitHub.")
else:
