...
def _pantalla_resultados(jornada, partidos, progreso, titular="RESULTADOS"):
    ...
    for i, m in enumerate(partidos[:10]):
        ...
        d.text((94, cy), _breve(m["home"])[:16], font=f_eq,
        ...
        d.text((ANCHO - 94, cy), _breve(m["away"])[:16], font=f_eq,
...
def _pantalla_clasificacion(tabla, progreso):
    ...
    for i, fila in enumerate(tabla[:20]):
        ...
        d.text((132, cy), _corto(nom)[:22], font=f_nom,
...
def _pantalla_pichichi(pichichis, progreso):
    ...
    f_pos = _f("Montserrat-Bold.ttf", 30)
    f_nom = _f("Montserrat-SemiBold.ttf", 38)
    f_eq = _f("Montserrat-Regular.ttf", 26)
    f_gol = _f("BebasNeue-Regular.ttf", 60)
    ...
    for i, s in enumerate(pichichis[:10]):
        ...
        color_pos = ORO if i == 0 else GRIS
        d.text((80, cy), str(i + 1), font=f_pos, fill=color_pos, anchor="lm")

        d.text((140, cy - 16), s["nombre"][:28], font=f_nom,
               fill=NEGRO, anchor="lm")
        d.text((140, cy + 24), _corto(s.get("equipo", ""))[:20], font=f_eq,
               fill=GRIS, anchor="lm")

        d.text((ANCHO - 80, cy + 4), str(s["goles"]), font=f_gol,
               fill=NEGRO, anchor="rm")
